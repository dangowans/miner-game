...
class World {
   ...
   * Gem and hazard density increases slightly with depth for variety.
   */
  _generateChunk(fromY) {
    const rng = this._rng;

    // Probability table – weights shift with depth for added variety
    const depthBonus = Math.floor(fromY / 50); // Every 50 rows, slight gem upgrade

    const TABLE = [
      { content: HIDDEN.NOTHING,  weight: Math.max(20, 38 - depthBonus) },
      { content: HIDDEN.GEM_LOW,  weight: Math.max(10, 30 - depthBonus) },
      { content: HIDDEN.GEM_MED,  weight: 15 + depthBonus              },
      { content: HIDDEN.GEM_HIGH, weight:  5 + depthBonus              },
    ...
    // Per-chunk item caps (rare items don't appear on every row)
